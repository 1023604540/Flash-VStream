    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if vision_tower is None or (images is None and features is None) or input_ids_o.shape[1] == 1:
            if past_key_values is not None and vision_tower is not None and (
                    (images is not None) or (features is not None)) and input_ids_o.shape[1] == 1:
        ...
        if (features is not None) or (type(images) is list) or (images.ndim == 5):
            ...
            image_features = self.compress_temporal_features(image_features,
                                                             new_input_embeds)  # [B, TP, D]  # Building memory by calling [compress_temporal_features] function
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            num_images = (cur_input_ids == IMAGE_TOKEN_INDEX).sum()  # count number of image tokens in the input_ids
            if num_images == 0:
                new_input_embeds[batch_idx] = torch.cat(
                    (new_input_embeds[batch_idx], image_features[cur_image_idx][0:0]), dim=0)
                cur_image_idx += 1
                continue
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    cur_new_labels.append(
                        torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=new_labels[0].device,
            ...
            assert cur_image_idx <= len(
                image_features), f"cur_image_idx ({cur_image_idx}) 超出 image_features 长度 ({len(image_features)})"
