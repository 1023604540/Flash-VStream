...
class VStreamMetaForCausalLM(ABC):
    ...
    def compress_temporal_features(self, image_features):
        ...
        new_image_features = []
        for img_feature in image_features:  # [T, P*P, D]
            # Initialize recurrent memory bank
            recurrent_memory_bank = []
            ...
            for segment_features in segments:
                ...
                recurrent_memory_bank.append(recurrent_memory)

            retrieved_index = self.retrieve_recurrent_memory(query, self.recurrent_memory_bank)
            retrieved_recurrent_memory = recurrent_memory_bank[retrieved_index]
            final_memory_feature = torch.cat(
                [Turing_memory_compreesed.flatten(0, 1), long_memory_compreesed.flatten(0, 1), cur_memory.flatten(0, 1),
                 retrieved_recurrent_memory.flatten(0, 1)], dim=0)
            new_image_features.append(final_memory_feature)
